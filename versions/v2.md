...
v2 is a clean break profile. A v2 decoder is not required to decode v1 payloads.

---

## 5. v2 Core Value Model

## 5.1 Purpose

v2 keeps schema-less usability but standardizes smaller default encodings for one-shot JSON payloads.
...
- v2 is the active wire profile for new implementations
- top-level message-local ids (`key_id`, `str_id`, `shape_id`) MUST reset per message
- unknown `key_ref` / `str_ref` / `shape_ref` ids MUST fail decode

## 17.2 v1 coexistence

- v2 decoders are not required to decode v1
- if an implementation supports both versions, it MUST use an explicit external version discriminator
- this specification does not define auto-detection between v1 and v2 payloads
